...
class RadiationPattern:
    ...
    def _phi_eval(self):
        return self._phi_raw * (np.pi / 180)

    def _f0(self, theta, phi):
        _left_side = self._k * self._len * (1 - np.sin(theta) * np.sin(self._phi_eval() + phi))
        _right_side = self._k * self._len * (1 - np.sin(theta) * np.sin(self._phi_eval() - phi))
    ...
    def _f2(self, theta, phi):
        return np.power((np.cos(phi) - np.sin(theta) * np.sin(self._phi_eval())) * self._f0(theta, phi), 2)
